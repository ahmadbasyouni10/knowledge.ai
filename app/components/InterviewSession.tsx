    ...
    try {
      ...
      const contextEnhancedSessionType = sessionType === 'mock'
        ? `You are an expert interviewer conducting a ${interviewFocus} for the position of ${topic}. 
           ...
           ${isSystemDesignInterview && systemToDesign ? `CRITICAL MEMORY DIRECTIVE: Always remember that we are designing a ${systemToDesign.toUpperCase()} system. Reference the candidate's previous answers and build on them. DON'T restart the topic or ask questions already answered. Maintain conversation continuity at all times.` : ''}
           ${isSystemDesignInterview && systemToDesign ? `QUESTION HANDLING: If the candidate asks a clarifying question like "Is this what you want?" or "Should I continue?", DO NOT restart the interview or repeat initial questions. Instead, DIRECTLY ANSWER their question and CONTINUE from where the conversation left off. NEVER forget what was already discussed.` : ''}
           ${isSystemDesignInterview && systemToDesign ? `CONTEXT MAINTENANCE: If the candidate has already provided information about functional/non-functional requirements or any other aspect, DO NOT ask for this information again. Instead, acknowledge what they've already shared and build upon it.` : ''}
           ${isSystemDesignInterview && systemToDesign ? `VERIFICATION RESPONSE: If the candidate asks for verification or confirmation, respond directly to their question while maintaining the flow of the interview. For example, if they ask "Is this what you meant?" reply with "Yes, that's exactly what I'm looking for" or provide a small correction, then CONTINUE with the next logical question based on previous context.` : ''}
